...
{
    public class AddCharterInEndConsumptionIssueVoucher : IAddCharterInEndConsumptionIssueVoucher
    {
       #region Prop
        ...
        public AddCharterInEndConsumptionIssueVoucher(IVoucherRepository voucherRepository,
        ...
        public void Execute(CharterIn charterIn, List<Issue> issues, string issueWarehouseCode, string issueNumber, long userId, string lineCode, string voyageCode,bool isReform = false)
        {
            try
            {
                ...
                issues.ForEach(c =>
                {
                    ...
                    var debiJournalEntry = CreateDebitJournalEntry(c, voucherSeting, charterIn, issueWarehouseCode, lineCode, voyageCode, isReform, charterIn.VesselInCompany.Code);
                    voucher.JournalEntrieses.Add(debiJournalEntry);

                    var creditJournalEntry = CreateCreditJournalEntry(c, voucherSeting, issueWarehouseCode, charterIn.VesselInCompany.Code, lineCode, voyageCode,isReform,charterIn);
        ...
        JournalEntry CreateDebitJournalEntry(Issue issue, VoucherSetingDetail voucherSeting, CharterIn charterIn, 
        string issueWarehouseCode, string lineCode, string voyageCode, bool isReform, string vesselCode)
        {
            ...
            {
                debiJournalEntry = CreateDebitJournalEntry(issue, voucherSeting, charterIn,
                    issueWarehouseCode, lineCode, voyageCode);
        ...
        JournalEntry CreateDebitJournalEntry(Issue issue, VoucherSetingDetail voucherSeting, CharterIn charterIn, 
            //<A.H> Parameters order changed.
            //string lineCode, string voyageCode, string issueWarehouseCode)
            string issueWarehouseCode, string lineCode, string voyageCode)
        {
            var debiJournalEntry = new JournalEntry();
            ...
            voucherSeting.DebitSegmentTypes.ForEach(c => debiJournalEntry.Segments.Add(CreateDebitSegment(
                          c, issueWarehouseCode, lineCode, voyageCode,charterIn)));
        ...
        JournalEntry CreateCreditJournalEntry(Issue issue, VoucherSetingDetail voucherSeting, string issueWarehouseCode,
           string vesselCode, string lineCode, string voyageCode,bool isReform,CharterIn charterIn)
        {
            var creditJournalEntry = new JournalEntry();
            if (isReform)
            {
                creditJournalEntry = CreateReformCreditJournalEntry(issue, voucherSeting, charterIn,
                    issueWarehouseCode, lineCode, voyageCode);
            }
            else
            {
        ...
        JournalEntry CreateReformCreditJournalEntry(Issue issue, VoucherSetingDetail voucherSeting, CharterIn charterIn,
           string issueWarehouseCode, string lineCode, string voyageCode)
        {
            var debiJournalEntry = new JournalEntry();
            debiJournalEntry.Segments = new List<Segment>();
        ...
        private Segment CreateDebitSegment(AsgnSegmentTypeVoucherSetingDetail segmentType, string issueWarehouseCode,
             string lineCode, string voyageCode, CharterIn charterIn)
        {
            var res = new Segment();
            res.Name = "x";
                ...
                case 2:
                    {
                        ...
                        if (charterIn.ChartererId== 5)
                        {
                            res.Code = "9999";
                        }
                        else if (charterIn.ChartererId == 2)
                        {
                            res.Code = "1102";
                        }
                    }
